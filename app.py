...
st.title("Ứng dụng xuất excel của Dương Kim Hiền")

col1, col2 = st.columns([2,1])
with col2:
    use_sample = st.checkbox("Dùng ảnh mẫu ", value=True)
    show_debug = st.checkbox("Thêm các ảnh mới", value=False)

uploaded = st.file_uploader("Upload ảnh (jpg/png). Ảnh càng rõ càng đúng nhé bạn", accept_multiple_files=True, type=["jpg","jpeg","png"])
...
if use_sample:
    if os.path.exists(SAMPLE_PATH):
        ...
        st.warning(f"Bạn nên bỏ tick ảnh mẫu đi vì ảnh mẫu không tồn tại ở {SAMPLE_PATH}")
    ...
    if st.button("Chạy OCR và Xuất Excel"):
        ...
        if not all_records:
            ...
            st.download_button("⬇ Tải Excel kết quả", data=buf,
                               file_name="Kim hien Cong chung.xlsx",
